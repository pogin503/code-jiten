...
if (isset($group_cd)) {
    ...
    $examples_stmt = $db->prepare(
        "SELECT example_id, example, language_id, language,
        group_cd, group_name FROM v_example_desc WHERE group_cd = :group_cd;"
    );
    ...
    <main>
        <div class="container p- 3">
            <div class="row px-3">
                <script id="json-vue" data-json="<?php echo h($json); ?>"></script>
                <script id="group-vue"
                        data-json="<?php echo ($group_cd == '') ? '' : h("{ \"group_cd\": ${group_cd}, \"group_name\": \"${group_name}\" }"); ?>">
                </script>
                <script id="disp-group-vue"
                        data-json="<?php echo ($disp_group == '') ? '{&quot;items&quot;: null, &quot;seen&quot;: false}' : h($disp_group); ?>">
                </script>
                <script id="group-names-vue" data-json="<?php echo h($group_data_json); ?>"></script>
                    ...
                    <tr v-for="item in items">
                        <td>
                            <a :href="'register.php?parent_id=' + item.parent_id">
                                {{ item.parent_name }}
                            </a> / <a :href="'register.php?group_cd=' + item.group_cd">
                                {{ item.group_name }}
                            </a>
                        </td>
                    </tr>
            ...
            <form name="save-form"
                ...
                <span v-for="item in group_names">
                    <span>
                        <a :href="'register.php?parent_id=' + item.group_cd">
                            [{{ item.group_name }}]
                        </a>
                    </span>
                </span>
                    ...
                    <tr v-for="(item, index) in items"
                        ...
                        <span v-if="item.insert_flag">
                            <select class="form-control"
                                    :name="'items[' + index + '][example][language_id]'"
                        ...
                        <span v-else>
                            ...
                            <input :name="'items[' + index + '][example][language]'"
                                   type="hidden"
                                   v-model="item.example.language"/>
                            <input :name="'items[' + index + '][example][language_id]'"
                                   type="hidden"
                                   v-model="item.example.language_id"/>
                        </span>
                        </td>
                        ...
                        <autosize-textarea
                            class="form-control"
                            :name="'items[' + index + '][example][example]'"
                            v-model="item.example.example" required>
                        </autosize-textarea>
                        </td>
                        <td>
                        <input class="form-control"
                               :name="'items[' + index + '][example][example_id]'"
                               type="hidden"
                               v-model.number="item.example.example_id"/>
                        <input :name="'items[' + index + '][group_cd]'"
                               type="hidden"
                               v-model.number="item.group_cd"/>
                        <input :name="'items[' + index + '][insert_flag]'"
                               type="hidden"
                               v-model="item.insert_flag"/>
                        <button class="btn btn-danger"
                                type="button"
                                v-on:click="remove(index, item.example.example_id)">
                            削除
                        </button>
                        </td>
                    </tr>
                ...
                <div style="display:none;">
                    ...
                    <input :name="'delete_target[]'"
                           :key="item.example_id"
                           type="number"
                           v-model.number="item.example_id"/>
                    </span>
                </div>
